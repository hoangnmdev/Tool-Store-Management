...
#=======================Title======================
        self.title = Label(root,bd=20, relief=RIDGE, text="COMPANY", fg="black", bg="white", font=("Time New Roman", 40, "bold"))
   ...
   # =======================DataFrame===================
    ...
    #=====================buttons frame===================
        showDataFrame=Frame(root, bd=20, relief=RIDGE)
...
#====================Search==========================
  ...
  # =========================DataFrameLeft=======================
        ...
        self.lblref3 = Label(DataframeLeft, font=("Time New Roman", 15, "bold"), text="Address: ", padx= 2, pady=6)
        self.lblref3.grid(row=3, column=0, sticky=W)
        self.txtref3=Entry(DataframeLeft,font=("Time New Roman", 15, "bold"), width=40, textvariable=self.Address)
        self.txtref3.grid(row=3, column=1)

        self.lblref4 = Label(DataframeLeft, font=("Time New Roman", 15, "bold"), text="City: ", padx= 2, pady=6)
        self.lblref4.grid(row=4, column=0, sticky=W)
        self.txtref4=Entry(DataframeLeft,font=("Time New Roman", 15, "bold"), width=40, textvariable=self.City)
        self.txtref4.grid(row=4, column=1)

        self.lblref5 = Label(DataframeLeft, font=("Time New Roman", 15, "bold"), text="Phone number: ", padx= 2, pady=6)
        ...
        self.lblref6 = Label(DataframeLeft, font=("Time New Roman", 15, "bold"), text="Email: ", padx= 2, pady=6)
        self.lblref6.grid(row=6, column=0, sticky=W)
        self.txtref6=Entry(DataframeLeft,font=("Time New Roman", 15, "bold"), width=40, textvariable=self.Email)
        self.txtref6.grid(row=6, column=1)

        self.lblref7 = Label(DataframeLeft, font=("Time New Roman", 15, "bold"), text="Website: ", padx= 2, pady=6)
        self.lblref7.grid(row=7, column=0, sticky=W)
        self.txtref7=Entry(DataframeLeft,font=("Time New Roman", 15, "bold"), width=40, textvariable=self.Website)
...
#=========================================ScrollBar======================================
        ...
        self.tree=ttk.Treeview(showDataFrame, column=("No", "ID", "Name", "Address", "City", "PhoneNumber", "Email", "Website")
    ...
    def registerButton(self):
            ...
            try:
                if self.ID.get()=="":
                    ...
                    cur.execute("Select * from company where ID=?",(self.ID.get(),))
                    row=cur.fetchone()
                    if row!=None:
                        messagebox.showerror("Error","ID already assigned")
                    else:
                        cur.execute("insert into company (ID , NAME , ADDRESS , CITY , PHONE_NUMBER , EMAIL , WEBSITE ) values(?,?,?,?,?,?,?)",(
                                    self.ID.get(),
                                    self.Name.get(),
                                    self.Address.get(),
                                    self.City.get(),
    ...
    def OnDoubleClick(self,ev):
        ...
        self.Website.set(row[7])
    
    def exitButton(self):
        self.exitButton = tkinter.messagebox.askyesno("Company Information","Confirm if you want to exit")
        ...
        try:
            if self.ID.get()=="":
                ...
                if row==None:
                    ...
                    cur.execute("UPDATE company SET NAME=? , ADDRESS=? , CITY=? , PHONE_NUMBER=? , EMAIL=? , WEBSITE=?  WHERE ID = ?",(
                                    self.Name.get(),
                                    self.Address.get(),
                                    self.City.get(),
                                    self.PhoneNumber.get(),
        ...
        try:
            if self.ID.get()=="":
                ...
                if row==None:
                    ...
                    if op==True:
                        cur.execute("DELETE FROM company WHERE ID=?", (self.ID.get(),))
                        conn.commit()
    ...
    def searchButton(self):
        ...
        cur=conn.cursor()
        try:
            cur.execute("SELECT * FROM COMPANY WHERE ID LIKE ?", ('%' + str(self.SEARCH.get()) + '%',))
            rows=cur.fetchall()
            self.tree.delete(*self.tree.get_children())
            for row in rows:    
                self.tree.insert('',END,values=row)
